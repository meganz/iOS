...
final class MeetingContainerViewModelTests: XCTestCase {
    ...
    func testAction_onViewReady() {
        ...
        let viewModel = MeetingContainerViewModel(router: router, chatRoom: chatRoom, call: call, callsUseCase: MockCallsUseCase(), callManagerUseCase: MockCallManagerUseCase(), userUseCase: MockUserUseCase(handle: 100))
    ...
    func testAction_hangCall_attendeeIsGuest() {
        ...
        let viewModel = MeetingContainerViewModel(router: router, chatRoom: chatRoom, call: call, callsUseCase: callUseCase, callManagerUseCase: callManagerUserCase, userUseCase: MockUserUseCase(handle: 100, hasUserLoggedIn: false))
    ...
    func testAction_hangCall_attendeeIsParticipantOrModerator() {
        ...
        let callUseCase = MockCallsUseCase()
        let callManagerUserCase = MockCallManagerUseCase()
        let viewModel = MeetingContainerViewModel(router: router, chatRoom: chatRoom, call: call, callsUseCase: callUseCase, callManagerUseCase: callManagerUserCase, userUseCase: MockUserUseCase(handle: 100))
        test(viewModel: viewModel, action: .hangCall(presenter: UIViewController()), expectedCommands: [])
