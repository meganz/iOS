...
class InviteContactViewController: UIViewController {
    ...
    @IBAction func moreButtonTapped(_ sender: Any) {
        let items = [NSLocalizedString("Hi, Have encrypted conversations on Mega with me and get 20GB+ free storage.", comment: "Text to send as SMS message to user contacts inviting them to MEGA"), userLink]
...
extension InviteContactViewController: ContactsPickerViewControllerDelegate {
    func contactsPicker(_ contactsPicker: ContactsPickerViewController, didSelectContacts values: [String]) {
        ...
        composeVC.body = NSLocalizedString("Hi, Have encrypted conversations on Mega with me and get 20GB+ free storage.", comment: "Text to send as SMS message to user contacts inviting them to MEGA") + " " + self.userLink
