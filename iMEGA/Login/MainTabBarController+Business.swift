...
public extension MainTabBarController {
    @objc func showBusinessAccountWarningIfNeeded () {
        
        if MEGASdkManager.sharedMEGASdk().businessStatus == .gracePeriod {
        ...
        if MEGASdkManager.sharedMEGASdk().businessStatus == .expired {
            let businessExpiredViewController = BusinessExpiredViewController()
            UIApplication.mnz_presentingViewController()?.present(businessExpiredViewController, animated: true, completion: nil)
        }
    }
}
