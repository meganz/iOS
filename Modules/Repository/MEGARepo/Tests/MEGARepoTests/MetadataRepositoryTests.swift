...
final class MetadataRepositoryTests: XCTestCase {
    ...
    func testCoordinateForVideo_withoutGPSInfo_shouldBeNil() throws {
        let url = try XCTUnwrap(Bundle.module.url(forResource: "sample_video_without_gps", withExtension: "mov"))
        let sut = MetadataRepository()
        let result = sut.coordinateForVideo(at: url)
        XCTAssertNil(result)
    }

    func testCoordinateForVideo_withGPSInfo_shouldReturnCoordinates() throws {
