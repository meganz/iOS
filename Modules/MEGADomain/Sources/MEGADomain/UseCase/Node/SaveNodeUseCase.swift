...
public struct SaveNodeUseCase<U: OfflineFilesRepositoryProtocol, V: FileCacheRepositoryProtocol, W: NodeRepositoryProtocol, Y: PhotosLibraryRepositoryProtocol, M: MediaUseCaseProtocol, P: PreferenceUseCaseProtocol, T: TransfersRepositoryProtocol>: SaveNodeUseCaseProtocol {
    ...
    public init(offlineFilesRepository: U, fileCacheRepository: V, nodeRepository: W, photosLibraryRepository: Y, mediaUseCase: M, preferenceUseCase: P, transferRepository: T) {
    ...
    public func saveNode(from transfer: TransferEntity, completion: @escaping (Result<Bool, SaveMediaToPhotosErrorEntity>) -> Void) {
        ...
        if savePhotoInGallery && mediaUseCase.isImage(name) || saveVideoInGallery && mediaUseCase.isVideo(name) || transferRepository.isSaveToPhotosAppTransfer(transfer) {
    ...
    private func urlForTransfer(transfer: TransferEntity, nodeName: String) -> URL? {
        ...
        if transferRepository.isSaveToPhotosAppTransfer(transfer) {
