...
final class TransferInventoryUseCaseTests: XCTestCase {
    func testTransfers_withoutFilteringUserTransfers() {
        let repo = MockTransferInventoryRepository.newRepo
        let sut = TransferInventoryUseCase(transferInventoryRepository: repo, fileSystemRepository: MockFileSystemRepository())
