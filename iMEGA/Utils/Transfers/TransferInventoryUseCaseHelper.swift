...
final class TransferInventoryUseCaseHelper: NSObject, Sendable {
    ...
    func completedTransfers() -> [MEGATransfer] {
        if let list = sdk.completedTransfers as? [MEGATransfer] {
            return list.filter {
                return !$0.isFolderTransfer && ($0.type == .upload || $0.path?.hasPrefix(fileSystem.documentsDirectory().path) ?? false || $0.appData?.contains(TransferMetaDataEntity.saveInPhotos.rawValue) ?? false || $0.appData?.contains(TransferMetaDataEntity.exportFile.rawValue) ?? false)
            }
        }
        return []
    }
