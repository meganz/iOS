        ...
        if MEGASdk.shared.isNode(inRubbish: node) {
            if let restoreNode = MEGASdk.shared.node(forHandle: node.restoreHandle),
               !MEGASdk.shared.isNode(inRubbish: restoreNode) {
                let restoreAction = swipeAction(image: UIImage.restore.withTintColor(.white), backgroundColor: UIColor.mnz_turquoise(for: traitCollection)) { [weak self] in
                    node.mnz_restore()
            ...
            let shareLinkAction = swipeAction(image: UIImage.link.withTintColor(.white), backgroundColor: UIColor.systemOrange) { [weak self] in
            ...
            let downloadAction = swipeAction(image: UIImage.offline.withTintColor(.white), backgroundColor: UIColor.mnz_turquoise(for: traitCollection)) { [weak self] in
            ...
            if cloudDrive?.displayMode != .backup {
                let rubbishBinAction = swipeAction(image: UIImage.rubbishBin.withTintColor(.white), backgroundColor: UIColor.mnz_red(for: traitCollection)) { [weak self] in
