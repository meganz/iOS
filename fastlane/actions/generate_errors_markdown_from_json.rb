  ...
  module Actions
    class GenerateErrorsMarkdownFromJsonAction < Action
      def self.run(params)
        ...
        return errors_markdown_generator.generate_markdown(params[:errors_json_path], params[:junit_file_path], params[:output_file_path])
      end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(key: :junit_file_path,
                                       description: "file path for the junit file generated by scan",
                                       verify_block: proc do |value|
                                          UI.user_error!("No file path given, pass using `junit_file_path: 'path'`") unless (value and not value.empty?)
                                       end),
