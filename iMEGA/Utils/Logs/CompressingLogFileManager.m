...
- (void)compressNextLogFile {
    ...
    MEGALogMax(@"CompressingLogFileManager: compressNextLogFile");
...
- (void)compressionDidSucceed:(DDLogFileInfo *)logFile {
    MEGALogMax(@"CompressingLogFileManager: compressionDidSucceed: %@", logFile.fileName);
...
- (void)compressionDidFail:(DDLogFileInfo *)logFile {
    MEGALogWarning(@"CompressingLogFileManager: compressionDidFail: %@", logFile.fileName);
...
- (void)didArchiveLogFile:(NSString *)logFilePath wasRolled:(BOOL)wasRolled {
    MEGALogMax(@"CompressingLogFileManager: didArchiveLogFile: %@ wasRolled: %@",
...
- (void)backgroundThread_CompressLogFile:(DDLogFileInfo *)logFile {
    @autoreleasepool {
        ...
        MEGALogInfo(@"CompressingLogFileManager: Compressing log file: %@", logFile.fileName);
        ...
        if (error) {
            ...
            MEGALogError(@"Compression of %@ failed: %@", inputFilePath, error);
            error = nil;
            BOOL ok = [[NSFileManager defaultManager] removeItemAtPath:tempOutputFilePath error:&error];
            if (!ok)
                MEGALogError(@"Failed to clean up %@ after failed compression: %@", tempOutputFilePath, error);
            ...
            if (!ok)
                MEGALogWarning(@"Warning: failed to remove original file %@ after compression: %@", inputFilePath, error);
