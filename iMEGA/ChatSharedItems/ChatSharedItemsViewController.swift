    ...
    @objc private func downloadTapped() {
        ...
        selectedMessages.forEach { message in
            if let node = message.nodeList?.node(at: 0) {
                transfers.append(CancellableTransfer(handle: node.handle, messageId: message.messageId, chatId: chatRoom.chatId, name: node.name, appData: nil, priority: false, type: .downloadChat))
            }
        }
...
extension ChatSharedItemsViewController: NodeActionViewControllerDelegate {
    func nodeAction(_ nodeAction: NodeActionViewController, didSelect action: MegaNodeActionType, for node: MEGANode, from sender: Any) {
        ...
        case .download:
            ...
            let transfer = CancellableTransfer(handle: node.handle, messageId: message.messageId, chatId: chatRoom.chatId, name: node.name, appData: nil, priority: false, isFile: node.isFile(), type: .downloadChat)
